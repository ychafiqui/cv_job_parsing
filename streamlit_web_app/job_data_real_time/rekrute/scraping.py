...
url = url_base + "offres.html?s=3&o=1&p=" # 50 jobs per page
# url = url_base + "offres-emploi-maroc.html" # 10 jobs per page
# url = url_base + "offres.html?clear=1&keyword=data%20scientist" # search for data scientist

source = "Rekrute"

# offres_pd = pd.read_csv("rekrute.csv")
# offres_pd = pd.DataFrame(columns=cols)

ids_to_skip = []

def scrape_rekrute(df_path, max_pages):
    try:
    ...
    while True:
        for i in range(1, max_pages + 1):
            try:
                ...
                continue
            soup = BeautifulSoup(page.text, "lxml", parse_only=SoupStrainer("ul", class_="job-list job-list2"))
            all_jobs = soup.find_all("li", class_="post-id")
            for job in all_jobs:
                company_logo = url_base + job.find("img", class_="photo").get("src")
                id = job.get("id")
                
                if not id in offres_pd["id"].astype(str).unique() and not id in ids_to_skip:
                    suffix_offre = id + "-emploi.html"
                    full_job_url = url_base + suffix_offre
