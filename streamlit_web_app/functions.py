...
def embedding_similarity(df):
    cv_hskills = st.session_state.info["hskills"]
    cv_edu = st.session_state.info["edu"]["diploma"]
    cv_profile = st.session_state.info["profile"]
    cv_infos = cv_hskills + cv_edu + [cv_profile]
    cv_infos = " ".join(cv_infos)
    cv_infos = cv_infos.lower()
    cv_infos = cv_infos.replace("\n", " ")

    doc1 = job_info_extr_eng(cv_infos)
    ...
    for row in df.itertuples():
        if type(row.hskills) == float and math.isnan(row.hskills): hskills = ""
        else: hskills = row.hskills.replace(", ", " ")
        if type(row.education) == float and math.isnan(row.education): education = ""
        else: education = row.education.replace("\n", " ")
        if type(row.profile) == float and math.isnan(row.profile): profile = ""
        else: profile = row.profile.replace("\n", " ")
        infos = hskills + " " + education + " " + profile
        doc2 = job_info_extr_eng(infos)
        df.loc[row.Index, "similarity"] = doc1.similarity(doc2)
...
def predict_salaire_cv_dh(desc, domaine, nb_years_exp, edu_lvl):
    if not domaine:
        domaine = classify_job(desc)
    domaine = le_domaine_dh.transform([domaine])
    desc = preprocess_desc_for_info(desc)
    ...
    # X = pd.DataFrame([[experience, nb_skills, nb_soft_skills, education_level, nb_lg]], 
    #     columns=["nb_exp", "nb_hard_skills", "nb_soft_skills", "education_level", "nb_langues"])
    X = pd.DataFrame([[nb_years_exp, nb_skills, nb_soft_skills, edu_lvl, nb_lg]], 
        columns=["nb_exp", "nb_hard_skills", "nb_soft_skills", "education_level", "nb_langues"])
    X["remote"] = desc.__contains__("|".join(remote_words_indicator))
    X["remote"] = X["remote"].astype(int)
    X["desc_len"] = len(desc)
...
def get_exp_months(info_exp):
    start_dates = info_exp["start_date"]
    end_dates = info_exp["end_date"]
    total_months = 0
...
def get_edu_level(info_edu):
    education = info_edu["diploma"]
    education = " ".join(education)
